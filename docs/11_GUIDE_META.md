...
| `business_management` | (선택) 비즈니스 자산 관리 |
...
4. **Permissions(권한 추가)** 에서 STEP 6의 스코프 체크:
   `instagram_basic`, `pages_show_list`, `pages_read_engagement`(필요 시 `instagram_manage_insights`).
5. **Generate Access Token** 클릭 → 페이스북 로그인/동의 팝업 → 권한 허용.
6. 생성된 긴 문자열이 **단기 사용자 토큰**(~1시간). 이걸 복사.
7. (확인) 탐색기 주소창에 `me/accounts?fields=instagram_business_account{id,username}` 실행 →
   `instagram_business_account.id` 가 나오면 IG 연결 성공. (이게 우리 앱이 추출할 `ig_user_id`)

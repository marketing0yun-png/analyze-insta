...
> ① **수집·지표 미터**(Meta·무료/시간할당량) — 수집과 지표를 **한 묶음**으로 본다. 지표(`/metrics`)는 Meta·LLM을 안 쓰고 적재된 raw 계산만 하지만, **티어 차등 지점**이라 수집과 같은 미터에 묶는다. ② **분석·비교 미터**(LLM·비용) — 콘텐츠 인사이트 분석과 비교분석이 한 풀을 공유.
...
- [ ] 사용 이력 테이블(user_id·행동·시각) + 2시간 윈도우 카운트(**수집·지표** / **분석·비교** 2개 미터)
- [ ] 티어 게이트: `/collect`·`/metrics`(수집·지표 미터·계정수 — 개인 토큰이면 무제한), `/analyze`·`/compare`(LLM 미터 공용 — 두 티어 모두 제한), `/accounts/self`(개인 토큰만), `/hashtags`(개인 토큰만)
- [ ] 크레딧/카운트다운 UI("다음 가능 10:34 — 1시간 12분 후" 실시간), 비활성 안내("개인 토큰 입력 필요")
      · 일괄 분석 시 **"비교분석용 1회는 남겨두세요"** 안내(분석·비교가 한 풀 공용이라 다 쓰면 비교 불가)

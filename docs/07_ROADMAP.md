...
## 현재 상태
🟢 **Phase 3.5(미터 코어) 사용량 미터 구현 완료(로컬, build/lint 통과 2026-06-09, D-024) — 사용자 검수 대기.**
...
- **선결 과제:** 오너 토큰 폴백 미구현 → 현재 전원 personal 티어로 판정(collect 한도 미발동). 외부 계정 개수·self/hashtags 개인전용 게이트, 비용/혼잡 안내 문구는 잔여.
...
- [ ] 마스터 콘솔(전체 데이터 조합 뷰) — **후속**
- [ ] **익명 → 구글 로그인 교체/연결**(link identity) — **후속**(Supabase Google OAuth 설정 동반)
- [ ] Meta 앱 검수(일반 공개 시) / 또는 테스터 한정 운영 결정 — **외부 차단**(휴대폰 인증 보류)
- [ ] 배포 전 보안 체크(`docs/06_AUTH_SECURITY.md` §5) — **후속**
...
- [ ] **잔여 게이트**: 외부 계정 개수 한도(체험 3 / 개인 10), `/accounts/self`·`/hashtags` 개인 토큰 전용 차단.
      ⚠️ **선결 과제 — 오너 토큰 폴백:** 현재 `/collect`·`/analyze` 는 *본인* credential 을 요구하므로 사실상 모든 사용자가 `personal` 티어다(= 체험 수집 한도가 아직 발동 안 함). "체험 유저가 오너 토큰으로 수집"을 구현해야 collect 게이트가 실제로 의미를 가진다. 미터 인프라는 그때 그대로 활성화됨.
- [x] 카운트다운 UI — **구현 완료**
...
- [ ] 잔여 안내 UI: 기능별 비활성("개인 토큰 입력 필요" — 내 계정 노출·도달 등), 일괄 분석 시 "비교분석용 1회는 남겨두세요" 안내(분석·비교 한 풀 공용)
- [ ] 해시태그 **신청** 테이블(날짜·키워드·user_id) + 마스터 일별 집계·추가(주 30개 쿼터 내) + 공통 노출
- [ ] LLM 비용 안내 문구(테스트 무료·관리자 부담·제한 양해·추후 완화·모델 업그레이드 예정)
- [ ] 혼잡 안내(수집 느려질 수 있음 → 개인 토큰 연결 유도)
- [ ] 크레딧 충전 수단 검토(행동 보상 / 보상형 광고 — ⚠️ 애드센스 인센티브 클릭 금지) · 쿠팡파트너스(구매 기반·대가성 표시 의무)

...
## 현재 상태
🟡 **기획·실현가능성 검토 완료. 문서화 완료. 코드 미착수.**
다음: 개발 착수 결정 → Phase 0(스캐폴딩).

---

## Phase 0 — 스캐폴딩 (착수 시)
- [ ] Next.js + Tailwind + shadcn/ui 초기화, PWA 설정(manifest/service worker)
- [ ] Supabase 프로젝트 생성, `.env.local` 구성(`docs/08_SETUP.md`)
- [ ] Vercel 연결
- [ ] Supabase 익명인증 활성화
- [ ] 기본 스키마 마이그레이션(`docs/04_DATA_MODEL.md`)
...
## 다음 할 일 (Next Action)
> **개발 착수 승인 대기 중.** 승인되면 Phase 0부터.
> 착수 시 가장 먼저: ① Meta 앱 생성+비즈니스 인증(리드타임), ② Supabase/Next.js 스캐폴딩 병행.

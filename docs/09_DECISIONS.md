...
- **검증:** `npm run lint`/`typecheck`/`build`(24 라우트) 통과.
- **후속(같은 날):** 사용량 미터를 **테스트 기간 정책 = 하루 10회·KST 0시 일괄 리셋**으로 전환(원래 2h 슬라이딩 5회 → 10회 상향 → 일일 전환). `usage-meter.ts`: `USAGE_WINDOW_MS` 슬라이딩 제거 → `kstDayStartMs()`(UTC+9 고정) 기준 당일 집계, `resetAt`=다음 KST 0시, 차단 문구 "한국시간 자정(0시)에 초기화". collect 개인 무제한 유지. 마스터 콘솔 집계도 2h → **오늘(KST)** 로 동기화(`collectToday`/`llmToday`). UI 문구(미터 카드·체험 안내·데모 목업 7/10·8/10) 동기화. 홈 계정 카드 헤더의 "비교 분석" 버튼 제거(CompareHeroCard CTA와 중복).
- **후속(실기기 피드백, 같은 날):** ① **포털 버그 수정** — 헤더 `backdrop-blur` 가 `fixed` 의 containing block 이 되어 모바일 바텀 시트가 56px 헤더 띠에 갇힘(패널 하단만 보이고 닫기 불가 + 스크롤 잠금) → `createPortal(document.body)` 렌더로 해결. ② 미연결 상태 표시를 주황 점 → **주의(`CircleAlert` ! 아이콘)** 로 — 사람 아이콘+점은 의미 전달이 약해 눌러볼 동기 부족. ③ 히어로 헤드라인 `<br>` 을 모바일 포함 **항상 강제 줄바꿈**(기존 `hidden sm:block` 은 모바일에서 "한눈에"가 자연 줄바꿈으로 찢어짐). ④ 헤더에 **카카오톡 문의 CTA**(카톡 옐로 `#FEE500` 원형 + 말풍선 아이콘, `http://pf.kakao.com/_xjdCpn/chat` 새 탭).
- **날짜:** 2026-06-10
---
## 미해결/추후 결정
